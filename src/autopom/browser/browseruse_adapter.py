    ...
    def __post_init__(self) -> None:
        ...
        if self.cdp_url:
            ...
            try:
                ...
                )  # might be None in persistent context
                if self._context.pages:
                    self._page = self._context.pages[-1]
                    self._page.bring_to_front()
                else:
                ...
                if "locked" in str(e).lower() or "singletonlock" in str(e).lower():
                    raise RuntimeError(
                        'Chrome profile is locked. Please completely close all existing Google Chrome windows (you can run `killall "Google Chrome"` in your terminal) before running with the Chrome Profile option.'
                    ) from e
                raise e
        else:
